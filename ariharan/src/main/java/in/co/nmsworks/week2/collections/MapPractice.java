...
public class MapPractice {
    ...
    public static void main(String[] args) {
        Map<Integer, List<String>> yearWithMovie = new HashMap<>();
        List<String> movieList = new ArrayList<>();
        MapPractice mp = new MapPractice();
        String[] data = mp.getDataAsArray();
        List<String> listOfMovie = new ArrayList<>(Arrays.asList(data));
        for (int i = 0; i < listOfMovie.size(); i++) {
            String moviewithYear = listOfMovie.get(i);
            String[] movie = moviewithYear.split("-");
            String movie1 = movie[0];
            Integer year = Integer.valueOf(movie[1].trim());
            if (yearWithMovie.containsKey(year)) {
                movieList = yearWithMovie.get(year);
                movieList=mp.addToList(movie1, movieList);

            } else {
                movieList = new ArrayList<>();
                movieList=mp.addToList(movie1, movieList);
            }
            yearWithMovie.put(year, movieList);
        }
        int count = 0;
        for (Map.Entry<Integer, List<String>> movies : yearWithMovie.entrySet()) {
    ...
    public List addToList(String movie, List<String> movies) {
        movies.add(movie);
        return movies;
