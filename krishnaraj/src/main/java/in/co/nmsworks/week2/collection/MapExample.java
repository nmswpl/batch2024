...
public class MapExample {
    public static void main(String[] args) {
        ...
        String dataArray[] = mapExample.getDataAsArray();
        List<String> dataList = new ArrayList<>();

        for (int i = 0; i < dataArray.length; i++) {
            dataList.add(dataArray[i]);
        }
        Map<Integer, List<String>> movieYearMap = new HashMap<>();
        List<String> movieNamesList = new ArrayList<>();

        for (String listData : dataList) {
            String[] movieNameYear = listData.split("-");

            if (movieYearMap.get(movieNameYear[0]).equals(null)){
                movieNamesList.add(movieNameYear[0]);
                movieYearMap.put(Integer.valueOf(movieNameYear[1]), movieNamesList);
            }
            else if(movieYearMap.get(movieNameYear[1]).equals(movieNameYear[1])) {
                movieNamesList.add(movieNameYear[0]);
                movieYearMap.replace(Integer.valueOf(movieNameYear[1]), movieNamesList);
            }
        }
    }
