...
public class MapPractice {
    ...
    public static void main(String[] args) {
        ...
        List<String> movies;
        Map<Integer, List<String>> movieListByYear = new HashMap<>();

        for (String s : data) {
            Integer key = Integer.valueOf(s.split("-")[1]);
            if (movieListByYear.containsKey(key)) {
                movies = movieListByYear.get(key);
                movies.add(s.split("-")[0]);
            } else {
                movies = new ArrayList<>();
                movies.add(s.split("-")[0]);
            }

            movieListByYear.put(key, movies);

        }

        System.out.println(movieListByYear);

        int count = 0;
        for (Integer year : movieListByYear.keySet()) {

            count += movieListByYear.get(year).size();
            System.out.println("Year: " + year + ", No. of Movies : " + movieListByYear.get(year).size());
        }
