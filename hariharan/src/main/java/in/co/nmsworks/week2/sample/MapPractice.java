...
public class MapPractice {
    public static void main(String[] args) {
        ...
        String[] data = mapPractice.getDataAsArray();

        List<String> movieData = new ArrayList<>();
        movieData.addAll(Arrays.asList(data));



        Map<Integer,List> mapOfMoviesData = new HashMap<>();
        List<String[]> moviesName = new ArrayList<>();


        for(String movieYear : movieData){
            int yearIndex = movieYear.indexOf("-");
            Integer year = Integer.parseInt(movieYear.substring(yearIndex+1,movieYear.length()));
            mapOfMoviesData.put(year,moviesName);

        }

        for(String moviesData : movieData){
            int yearIndex = moviesData.indexOf("-");
            Integer year = Integer.parseInt(moviesData.substring(yearIndex+1,moviesData.length()));
            String movieNames = moviesData.substring(0,yearIndex);
